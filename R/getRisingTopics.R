#' Extracts information about rising topics associated with the requested search term
#'
...
#' @examples
#' \dontrun{
#' getRisingTopics("apple", api.key = "YOUR_API_KEY")
#'
#' getRisingTopics(terms="Amazon", geo = "US", property = "news",
#' startDate = "2014-01", endDate = "2014-12", api.key = "YOUR_API_KEY")
#' }



getRisingTopics <- function(terms, geo=NULL, property=NULL, category=NULL, startDate=NULL, endDate=NULL, api.key) {

  #Base link of API call
  link <- "https://www.googleapis.com/trends/v1beta/risingTopics?"
  ...
  if(httr::http_error(f) == T){
    ...
    if(length(d)==0){
      ...
      names(res)<-c("topicTitle", "topicId", "value", "isBreakout")
    }else{
