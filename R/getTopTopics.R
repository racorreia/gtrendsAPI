#' Extracts information about the top topics associated with the requested search term
#'
...
#' @examples
#' \dontrun{
#' getTopTopics("apple", api.key = "YOUR_API_KEY")
#'
#' getTopTopics(terms="Amazon", geo = "US", property = "news",
#' startDate = "2014-01", endDate = "2014-12", api.key = "YOUR_API_KEY")
#' }



getTopTopics <- function(terms, geo=NULL, property=NULL, category=NULL, startDate=NULL, endDate=NULL, api.key) {

  #Base link of API call
  link <- "https://www.googleapis.com/trends/v1beta/topTopics?"
  ...
  if(httr::http_error(f) == T){
    ...
    if(length(d)==0){
      ...
      names(res)<-c("topicTitle", "topicId", "value")
