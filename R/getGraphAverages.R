#' Extracts Google Trends average search interest data associated with multiple topics
#'
#' @param terms The search terms of interest. A minimum of two and Up to five search terms can be queried simultaneously.
...
#' @return Returns the average search volume of a Google Trends time series.
#' @export
#'
#' @examples
#' \dontrun{
#' getGraphAverages(c("apple","orange"), api.key = "YOUR_API_KEY")
#'
#' getGraphAverages(c("Toyota","Fiat","Chevrolet"), geo = "US", property = "news",
#' category = "Car Video", startDate = "2014-01", endDate = "2014-12", api.key = "YOUR_API_KEY")
#' }



getGraphAverages <- function(terms, geo=NULL, property=NULL, category=NULL, startDate=NULL, endDate=NULL, api.key) {

  #Base link of API call
  link <- "https://www.googleapis.com/trends/v1beta/graphAverages?"
  ...
  if (length(terms) > 5){
    stop("terms should have no more than five elements", call. = FALSE)
  }

  if (length(terms) == 1){
    stop("To extract average data, terms should have at least two elements", call. = FALSE)
  }

  if (length(terms) > 1 & length(terms) <= 5){
    link <- paste0(link, "terms=", paste0(terms,collapse = "&terms="))
  }
  ...
  names(res)<-c("keyword", "average volume")
