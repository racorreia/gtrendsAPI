#' Extracts Google Trends search interest data per country
#'
...
#' @examples
#' \dontrun{
#' getRegions("apple", api.key = "YOUR_API_KEY")
#'
#' getRegions(c("apple","pear","orange"), geo = "US", property = "news",
...
getRegions <- function(terms, geo=NULL, property=NULL, category=NULL, startDate=NULL, endDate=NULL, api.key) {

  #Base link of API call
  link <- "https://www.googleapis.com/trends/v1beta/regions?"
  ...
  if(httr::http_error(f) == T){
    ...
    if(length(d)==0){
      ...
      names(res)<-c("regionCode", "regionName", "value")
