#' Extracts information about rising queries associated with the requested search term
#'
...
#' @return Returns up to 25 rising queries associated with the provided search term or topic.
#' @export
#'
#' @examples
#' \dontrun{
#' getRisingQueries("apple", api.key = "YOUR_API_KEY")
#'
#' getRisingQueries(terms="Amazon", geo = "US", property = "news",
#' startDate = "2014-01", endDate = "2014-12", api.key = "YOUR_API_KEY")
#' }



getRisingQueries <- function(terms, geo=NULL, property=NULL, category=NULL, startDate=NULL, endDate=NULL, api.key) {

  #Base link of API call
  link <- "https://www.googleapis.com/trends/v1beta/risingQueries?"
  ...
  if(httr::http_error(f) == T){
    ...
    if(length(d)==0){
      ...
      names(res)<-c("risingSearches", "value", "isBreakout")
